...
const deployWithSigner = async (environment: string, signer: Signer, uniswapRouterAddress: string, unitradeTokenAddress: string) => {
  try {
    const unitradeStaker: Contract = await deployContract(signer, UniTradeStaker01, [unitradeTokenAddress])
    logContractData(environment, "UniTradeStaker01", unitradeStaker)
    const unitradeIncinerator: Contract = await deployContract(signer, UniTradeIncinerator, [uniswapRouterAddress, unitradeTokenAddress])
    logContractData(environment, "UniTradeIncinerator", unitradeIncinerator)
    const unitradeOrderBook: Contract = await deployContract(signer, UniTradeOrderBook, [
    ...
    logContractData(environment, "UniTradeOrderBook", unitradeOrderBook)
    const uniTradeMarketOrders = await deployContract(signer, UniTradeMarketOrders, [unitradeOrderBook.address])
    logContractData(environment, "UniTradeMarketOrders", uniTradeMarketOrders)
  } catch (e) {
    console.error(e)
  }
}
