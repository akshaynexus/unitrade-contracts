...
describe("UniTradeOrderBook T2E", () => {
  ...
  describe("places a regular token order for eth", () => {
    ...
    beforeEach(async () => {
      params1 = [orderType.TokensForEth, tokenA.address, testWeth.address, 1000, 2000, 5000]
    })
    ...
    describe("places an invalid order", () => {
      ...
      it("with an output token that is not acceptable", async () => {
        await expect(orderBook.callStatic.placeOrder(orderType.TokensForEth, tokenA.address, tokenB.address, 1000, 2000, 5000, { value: 5000 })).to.be.revertedWith("Token out must be WETH")
      })
      ...
      describe("places order", () => {
        ...
        it("emits an event", async () => {
          await expect(receipt).to.emit(orderBook, "OrderPlaced")
            .withArgs(0, 2, wallet.address, tokenA.address, testWeth.address, 1000, 2000, 5000, 5000)
        })
        ...
        describe("updates an order", () => {
          ...
          describe("with insufficient value", () => {
            it("should be reverted", async () => {
              await expect(orderBook.callStatic.updateOrder(0, 2000, 4000, 6000, { value: 500 })).to.be.revertedWith("Additional fee must match")
            })
          })

          describe("without token pre-approval", () => {
            it("should be reverted", async () => {
              await expect(orderBook.callStatic.updateOrder(0, 2000, 4000, 6000, { value: 1000 })).to.be.revertedWith("TransferHelper: TRANSFER_FROM_FAILED")
            })
          })
          ...
          describe("with additional deposit", () => {
            ...
            it("has expected order state", async () => {
              ...
              expect(response.amountOutExpected.toNumber()).to.equal(4000)
          ...
          describe("with refundable amount", () => {
            ...
            beforeEach(async () => {
              ...
              receipt = orderBook.updateOrder(0, 500, 1000, 3000)
              ret = await receipt
            })

            it("emits an event", async () => {
              await expect(receipt).to.emit(orderBook, "OrderUpdated").withArgs(0, 500, 1000, 3000)
            })
            ...
            it("has expected order state", async () => {
              ...
              expect(response.amountOutExpected.toNumber()).to.equal(1000)
        ...
        describe("executes an order", () => {
          ...
          describe("calling statically", () => {
            it("returns swap amounts", async () => {
              ...
              expect(response[1]).to.equal(2000)
            })
          })
  ...
  describe("places a fee charging token order for eth", () => {
    ...
    beforeEach(async () => {
      params1 = [orderType.TokensForEth, tokenC.address, testWeth.address, 1000, 2000, 5000]
    })
    ...
    describe("places an invalid order", () => {
      ...
      it("with an output token that is not acceptable", async () => {
        await expect(orderBook.callStatic.placeOrder(orderType.TokensForEth, tokenC.address, tokenB.address, 1000, 2000, 5000, { value: 5000 })).to.be.revertedWith("Token out must be WETH")
      })
    ...
    describe("places a valid order", () => {
      ...
      describe("places order", () => {
        ...
        it("emits an event", async () => {
          await expect(receipt).to.emit(orderBook, "OrderPlaced")
            .withArgs(0, 2, wallet.address, tokenC.address, testWeth.address, 990, 2000, 5000, 5000)
        })
        ...
        describe("updates an order", () => {
          ...
          describe("with additional deposit", () => {
            ...
            it("emits an event", async () => {
              await expect(receipt).to.emit(orderBook, "OrderUpdated").withArgs(0, 1990, 4000, 6000)
            })
